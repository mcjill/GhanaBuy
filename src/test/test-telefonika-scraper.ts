import { telefonikaScraper } from '../lib/scrapers/telefonika.js';
...
async function testTelefonikaScraper() {
  ...
  try {
    ...
    const result = await telefonikaScraper.scrape({ query, minBudget, maxBudget });
    ...
    if (result.success && result.products.length > 0) {
      ...
      products.forEach((product, index) => {
        ...
        if (product.metadata?.relevancyScore !== undefined) {
          console.log(`Scraper Relevancy Score: ${(product.metadata.relevancyScore * 100).toFixed(1)}%`);
        }
      });
...
// Run the test
console.log('Running Telefonika scraper quality test...\n');
testTelefonikaScraper().catch(console.error);
